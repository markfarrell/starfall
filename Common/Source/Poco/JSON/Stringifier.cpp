...
void Stringifier::stringify(const Var& any, std::ostream& out, unsigned int indent)
{
	...
	else if ( any.isString() )
	{
		...
		bool quote = true; //whether or not to quote the value of the dynamic var with ""; usually this should be true to comply to the JSON standard.
		std::string value = any.convert<std::string>();

		//Syntax: use <% value %> to disable quotations
		if(value.substr(0,2) == "<%" && value.substr(value.size()-2, 2) == "%>") {
			quote = false;
			value = value.substr(2, value.size()-2);
		};
